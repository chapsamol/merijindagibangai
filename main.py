...
# Ultra-Fast Event Loop Optimization
try:
    import uvloop
...
# Shared State (Synchronized with Dashboard UI)
RAM_STATE = {
    "kite": None, "kws": None,
...
@app.get("/api/stats/")
async def get_stats():
    # Sync PnL and Engine Status
    total = 0.0
    for side in ["bull", "bear", "mom_bull", "mom_bear"]:
        side_pnl = sum(t.get('pnl', 0) for t in RAM_STATE["trades"][side])
        RAM_STATE["pnl"][side] = side_pnl
        total += side_pnl
    RAM_STATE["pnl"]["total"] = total
...
async def get_orders():
    # Return active trades for the UI table
    return RAM_STATE["trades"]

@app.get("/api/scanner/")
async def get_scanner():
    # Detect pre-trigger signals for the Left Sidebar
...
async def kite_callback(request_token: str):
    try:
        ...
        # (Ticker event binding same as before)
        return HTMLResponse("<h2>Auth Successful!</h2><script>window.close()</script>")
