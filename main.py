...
# Speed optimization (2-3x faster than default loop)
try:
    import uvloop
...
# --- STOCK UNIVERSE ---
# PASTE YOUR FULL STOCK_INDEX_MAPPING HERE
STOCK_INDEX_MAPPING = {
    'MRF': 'NIFTY 500',
    'PAGEIND': 'NIFTY 500',
    'BOSCHLTD': 'NIFTY 500',
...
def on_ticks(ws, ticks):
    for tick in ticks:
        ...
        if token in RAM_STATE["stocks"]:
            ltp, vol = tick['last_price'], tick.get('volume_traded', 0)
...
# --- STARTUP LOGIC: Auto-restore from Redis ---
@app.on_event("startup")
async def startup_event():
    key, secret = TradeControl.get_config()
    token = TradeControl.get_access_token()
    
    if key and secret:
        RAM_STATE["api_key"] = key
        RAM_STATE["api_secret"] = secret
        logger.info("API Credentials loaded from Redis.")
    ...
    if token and key:
        try:
            ...
            # Auto-init instruments mapping
            instruments = RAM_STATE["kite"].instruments("NSE")
            for instr in instruments:
                symbol = instr['tradingsymbol']
                if symbol in STOCK_INDEX_MAPPING:
                    token_id = instr['instrument_token']
                    RAM_STATE["stocks"][token_id] = {
...
async def kite_callback(request_token: str = None):
    try:
        api_key = RAM_STATE["api_key"]
        api_secret = RAM_STATE["api_secret"]
        kite = KiteConnect(api_key=api_key)
        ...
        TradeControl.save_access_token(token)
...
async def get_stats():
    ...
    for side in ["bull", "bear", "mom_bull", "mom_bear"]:
        ...
        engine_pnls[side] = side_pnl
        total_pnl += side_pnl
    
    RAM_STATE["pnl"]["total"] = total_pnl
    return {
        "pnl": {**RAM_STATE["pnl"], **engine_pnls},
...
async def control_center(data: dict):
    ...
    if action == "save_api":
        ...
        TradeControl.save_config(data["api_key"], data["api_secret"])
