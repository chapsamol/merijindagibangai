...
# SCANNER_VOL_SMA_MULT_REQ = 2.0      # volume >= 2 * SMA  (UPDATED)
...
# # -----------------------------
# # ✅ Scanner (sticky) logic (UPDATED as per your request)
# # -----------------------------
...
#         # reset per-stock scanner cross tracking (safe, doesn't affect engines)
#         try:
#             stocks = ENGINE_STATE.get("stocks", {}) or {}
#             for _tok, st in stocks.items():
#                 if isinstance(st, dict):
#                     st.pop("_scanner_prev_close", None)
#         except Exception:
#             pass
...
#         # sort: latest highlight first, then latest update
#         lst.sort(
#             key=lambda x: (
#                 float(x.get("last_highlight_epoch") or 0.0),
#                 float(x.get("last_update_epoch") or 0.0),
#             ),
#             reverse=True,
#         )
...
#     # IMPORTANT: keep sticky rows, but order by highlight-recency (not update-recency)
#     lst.sort(
#         key=lambda x: (
#             float(x.get("last_highlight_epoch") or 0.0),
#             float(x.get("last_update_epoch") or 0.0),
...
#     """
#     Updated rules (your request):
#       ✅ Detect & highlight:
#          - BULL breakout of previous day HIGH (PDH)
#          - BEAR breakout of previous day LOW  (PDL)
#       ✅ Breakout candle must satisfy:
#          - turnover >= ₹1 crore
#          - volume >= 2 * volume SMA
#       ✅ Once breakout happens at least once (QUALIFIED), keep it in scanner for whole day (sticky)
#       ✅ If it breaks again later (crosses again) with criteria, highlight again and move to top
#       ✅ Data keeps updating on every candle close
#     """
#     _ensure_scanner_day_rollover()
...
#     high_px = float(candle.get("high") or close_px)
#     low_px = float(candle.get("low") or close_px)
...
#     sma = float(st.get("sma") or 0.0)  # assumed Volume SMA

#     # ---- Detect breakout as a CROSS (to allow re-highlight on whipsaws) ----
#     prev_close = float(st.get("_scanner_prev_close") or 0.0)

#     bull_cross = False
#     bear_cross = False

#     if pdh > 0:
#         # cross above PDH: was below, now touched/above
#         if prev_close > 0:
#             bull_cross = (prev_close < pdh) and (high_px >= pdh)
#         else:
#             bull_cross = (high_px >= pdh)

#     if pdl > 0:
#         # cross below PDL: was above, now touched/below
#         if prev_close > 0:
#             bear_cross = (prev_close > pdl) and (low_px <= pdl)
#         else:
#             bear_cross = (low_px <= pdl)

#     # Save for next candle
#     st["_scanner_prev_close"] = close_px

#     breakout_event = bull_cross or bear_cross
...
#             side = "bull"
#             breakout_type = "BULL_PDH_BREAK"
#             breakout_level = pdh
#         elif bear_cross and not bull_cross:
#             side = "bear"
#             breakout_type = "BEAR_PDL_BREAK"
#             breakout_level = pdl
#         else:
#             # Very rare (both crossed in same candle): pick based on close / excursion
...
#                 breakout_level = pdl
#             else:
#                 up_ex = (high_px - pdh) if pdh > 0 else -1e9
#                 dn_ex = (pdl - low_px) if pdl > 0 else -1e9
#                 if up_ex >= dn_ex:
#                     side = "bull"
#                     breakout_type = "BULL_PDH_BREAK"
#                     breakout_level = pdh
...
#     # If not breakout candle: update only if already sticky (bull/bear)
#     if side is None:
#         existing_side = None
#         existing_row = None

#         for s in ("bull", "bear"):
#             for item in (STATE.scanner.get(s) or []):
...
#         side = existing_side
#         breakout_type = str(existing_row.get("breakout_type")) if isinstance(existing_row, dict) else None
#         breakout_level = float(existing_row.get("breakout_level") or 0.0) if isinstance(existing_row, dict) else 0.0

#     # ---- Breakout candle criteria (ONLY used to highlight + first-time inclusion) ----
#     turnover = float(close_px) * float(vol_candle)
#     vol_mult = (float(vol_candle) / float(sma)) if sma and sma > 0 else 0.0

#     qualifies_candle = (turnover >= float(SCANNER_TURNOVER_MIN)) and (vol_mult >= float(SCANNER_VOL_SMA_MULT_REQ))
#     qualifies_highlight = bool(breakout_event and qualifies_candle)

#     is_sticky = await _scanner_is_sticky(side, symbol)

#     # If not sticky and not a NEW qualified breakout highlight -> don't include
#     if (not is_sticky) and (not qualifies_highlight):
#         return
...
#     first_seen = str(existing.get("first_seen")) if isinstance(existing, dict) and existing.get("first_seen") else now_str
#     first_seen_epoch = float(existing.get("first_seen_epoch")) if isinstance(existing, dict) and existing.get("first_seen_epoch") else now_epoch

#     prev_last_highlight_epoch = float(existing.get("last_highlight_epoch") or 0.0) if isinstance(existing, dict) else 0.0
#     prev_highlight_count = int(existing.get("highlight_count") or 0) if isinstance(existing, dict) else 0

#     last_highlight_epoch = prev_last_highlight_epoch
#     highlight_count = prev_highlight_count
#     highlight = False

#     if qualifies_highlight:
#         highlight = True
#         last_highlight_epoch = now_epoch
#         highlight_count = prev_highlight_count + 1
#     elif not last_highlight_epoch:
#         last_highlight_epoch = prev_last_highlight_epoch

#     side_label = "BULL" if side == "bull" else "BEAR"
#     level_label = "PDH" if side == "bull" else "PDL"
#     lvl = breakout_level

#     if qualifies_highlight:
#         reason = (
#             f"{side_label} BREAK {level_label} {lvl:.2f} ✅ | "
...
#         # breakout info
#         "breakout_type": breakout_type or ("BULL_PDH_BREAK" if side == "bull" else "BEAR_PDL_BREAK"),
#         "breakout_level": float(lvl or 0.0),
#         "pdh": float(pdh or 0.0),
#         "pdl": float(pdl or 0.0),

#         # candle info
#         "candle_open": float(candle.get("open") or 0.0),
#         "candle_high": float(high_px),
#         "candle_low": float(low_px),
#         "candle_close": float(close_px),
#         "candle_volume": int(vol_candle),

#         # metrics
#         "volume_sma": float(sma or 0.0),
#         "volume_mult": round(float(vol_mult), 4),
#         "turnover": round(float(turnover), 2),

#         # sticky + highlight behaviour
#         "highlight": bool(highlight),
#         "highlight_count": int(highlight_count),
#         "last_highlight_epoch": float(last_highlight_epoch),

#         # timestamps
#         "seen_time": now_str,
#         "first_seen": first_seen,
#         "first_seen_epoch": float(first_seen_epoch),
#         "last_update_epoch": float(now_epoch),

#         # keep your existing trigger_px field (for UI compatibility)
#         "trigger_px": float(close_px),
...
async def _scanner_load_from_redis() -> None:
    ...
    for side in ("bull", "bear"):
        ...
        # ✅ highlighted first, then recent
        lst.sort(
            key=lambda x: (
                float(x.get("last_highlight_epoch") or 0),
                float(x.get("last_update_epoch") or 0),
            ),
            reverse=True,
        )
        STATE.scanner[side] = lst[:400]
...
async def _scanner_upsert(side: str, symbol: str, row: Dict[str, Any]) -> None:
    ...
    make_sticky = bool(row.get("make_sticky", False))

    # Redis (only mark sticky when explicitly requested)
    try:
        if make_sticky and hasattr(TradeControl, "scanner_add_sticky"):
            await TradeControl.scanner_add_sticky(side, symbol)
        if hasattr(TradeControl, "scanner_upsert_row"):
            await TradeControl.scanner_upsert_row(side, symbol, row)
    except Exception:
    ...
    # ✅ highlighted first, then update time
    lst.sort(
        key=lambda x: (
            float(x.get("last_highlight_epoch") or 0),
            float(x.get("last_update_epoch") or 0),
...
def _crossover_bull(pdh: float, o: float, h: float, l: float, c: float) -> bool:
    """
    Bull breakout cross detection for THIS candle.
    We treat as "cross" if candle traded from below PDH to at/above PDH.
    """
    if pdh <= 0:
        return False
    # from below to above within candle
    if o < pdh < c:
        # ensure it actually came from below at some point
        return True
    # # optional: gap-open above PDH (still breakout on this candle)
    # if o >= pdh:
    #     return True
    return False


def _crossover_bear(pdl: float, o: float, h: float, l: float, c: float) -> bool:
    """
    Bear breakdown cross detection for THIS candle.
    Cross if candle traded from above PDL to at/below PDL.
    """
    if pdl <= 0:
        return False
    if l < pdl < o:
...
async def _process_breakout_scanner_from_candle(token: int, candle: Dict[str, Any]) -> None:
    """
    ✅ REQUIRED BEHAVIOR (FINAL):
      - highlight triggers ONLY if BOTH happen on SAME current candle:
          1) price crosses PDH/PDL now
          2) candle meets turnover + volume×SMA now
      - if highlight happened at least once today => keep the stock for whole day (sticky)
      - later candles can keep updating row (even without qualify/cross)
      - if same stock again produces a qualifying breakout candle => highlight again and bubble to top
    """
    _ensure_scanner_day_rollover()
    ...
    if c_close <= 0 or vol_candle < 0:
        return

    # if open/high/low missing (shouldn't), fallback to close
    if c_open <= 0:
        c_open = c_close
    if c_high <= 0:
        c_high = c_close
    ...
    turnover = float(c_close) * float(max(vol_candle, 0))
    vol_mult = (float(vol_candle) / float(sma)) if sma and sma > 0 else 0.0
    qualifies = (turnover >= float(SCANNER_TURNOVER_MIN)) and (vol_mult >= float(SCANNER_VOL_SMA_MULT_REQ))

    crossed_bull = _crossover_bull(pdh, c_open, c_high, c_low, c_close)
    crossed_bear = _crossover_bear(pdl, c_open, c_high, c_low, c_close)

    crossed_level = crossed_bull or crossed_bear

    # Determine side only when crossing candle; otherwise we only update if sticky.
    side: Optional[str] = None
    breakout_level: float = 0.0
    breakout_type: str = ""
    ...
    if crossed_bull:
        ...
        breakout_type = "PDH_BREAKOUT"
    elif crossed_bear:
        side = "bear"
        breakout_level = float(pdl)
        breakout_type = "PDL_BREAKDOWN"

    # Sticky check (only bull/bear scanners)
    sticky_bull = await _scanner_is_sticky("bull", symbol)
    sticky_bear = await _scanner_is_sticky("bear", symbol)

    # If not crossing now, still update only if already sticky.
    if side is None:
        if sticky_bull:
            side = "bull"
        elif sticky_bear:
    ...
    is_sticky = (side == "bull" and sticky_bull) or (side == "bear" and sticky_bear)

    # ✅ HIGHLIGHT EVENT: must be on THIS candle: (cross now) AND (qualifies now)
    highlight_event = bool(crossed_level and (side in ("bull", "bear")) and qualifies and ((side == "bull" and crossed_bull) or (side == "bear" and crossed_bear)))

    # If not sticky yet and this is NOT a highlight event => do not add to scanner
    if not is_sticky and not highlight_event:
        return
    ...
    # Preserve first_seen + counts if present already in STATE
    first_seen = now_str
    first_seen_epoch = now_epoch
    highlight_count = 0
    last_highlight_epoch = 0.0
    last_highlight_time = ""
    ...
    if prev_row:
        first_seen = str(prev_row.get("first_seen") or first_seen)
        first_seen_epoch = float(prev_row.get("first_seen_epoch") or first_seen_epoch)
        highlight_count = int(prev_row.get("highlight_count") or 0)
        last_highlight_epoch = float(prev_row.get("last_highlight_epoch") or 0.0)
        last_highlight_time = str(prev_row.get("last_highlight_time") or "")

        # if breakout_level/type not set (non-cross update), keep previous
        if not breakout_level:
            try:
                breakout_level = float(prev_row.get("breakout_level") or 0.0)
            except Exception:
                breakout_level = 0.0
        if not breakout_type:
            breakout_type = str(prev_row.get("breakout_type") or "")

    # bump highlight stats only on highlight event
    if highlight_event:
        highlight_count += 1
        last_highlight_epoch = now_epoch
        last_highlight_time = now_str

    reason_bits = []
    if breakout_type:
        reason_bits.append(breakout_type.replace("_", " "))
    if highlight_event:
        reason_bits.append("✅ VOL CONFIRM")
    elif is_sticky:
        reason_bits.append("STICKY")

    reason_bits.append(f"TURNOVER ₹{turnover:,.0f}")
    reason_bits.append(f"VOLx {vol_mult:.2f} (SMA {sma:,.0f})")
    ...
    row = {
        ...
        # highlight controls (UI uses these)
        "highlight": bool(highlight_event),
        "highlight_count": int(highlight_count),
        "last_highlight_epoch": float(last_highlight_epoch),
        "last_highlight_time": str(last_highlight_time),

        # sticky control (only when highlight event creates stickiness)
        "make_sticky": bool(highlight_event),
