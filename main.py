...
# # main.py  (FULL WORKING + INTEGRATED FIXES)
# #
# # ✅ Fixes your current error: "❌ [BRK] <SYMBOL> dhan session missing"
# # ✅ Integrates BreakoutEngine (tick-fast run + candle-close qualification)
# # ✅ Adds 1-minute candle builder from ticks (calls BreakoutEngine.on_candle_close)
# # ✅ Adds Dhan Order Update websocket worker + status in /api/stats
# # ✅ Makes credential loading robust (TradeControl + direct Redis fallback)
# # ✅ Auto re-initializes dhan client if token/clientId arrive later (Heroku startup race)
# #
# # IMPORTANT (Heroku):
# #   Use ONE worker only (otherwise multiple websockets & random disconnects):
# #     web: uvicorn main:app --host 0.0.0.0 --port $PORT --workers 1
# #
# # Requirements:
# #   pip install fastapi uvicorn websockets pytz redis dhanhq
...
# # WS keepalive (DO NOT disable on Heroku)
# WS_PING_INTERVAL = float(os.getenv("WS_PING_INTERVAL", "20"))
# WS_PING_TIMEOUT = float(os.getenv("WS_PING_TIMEOUT", "20"))

# # Auto reinit dhan client every N seconds if missing
...
#         # cached market data from redis: token->dict (pdh/pdl/sma/etc)
#         self.market_data: Dict[int, Dict[str, Any]] = {}
...
# # -----------------------------
# ENGINE_STATE: Dict[str, Any] = {
#     "stocks": {},  # token -> stock dict (must include symbol/security_id/pdh/pdl/sma)
#     "config": {},  # side -> config dict
#     "engine_live": {},  # side -> bool (MUST be bool)
#     "trades": {"bull": [], "bear": []},
#     "manual_exits": set(),
...
# # Per-token locks (BreakoutEngine comment expects locking outside)
# TOKEN_LOCKS: Dict[int, asyncio.Lock] = {}
...
#     2) Fallback to direct redis keys used by your /api/control fallback writes
#     """
#     client_id = ""
#     access_token = ""
...
#         # last fallback: sometimes token is mistakenly stored in api_secret
#         if not access_token:
#             try:
...
#       nexus:universe:symbols
#     """
#     # 1) Try TradeControl helper if present
#     try:
#         fn_tokens = getattr(TradeControl, "get_subscribe_universe_tokens", None)
#         if callable(fn_tokens):
#             tokens = await fn_tokens()
...
# # -----------------------------
# # Dhan client init (FIX for "dhan session missing")
# # -----------------------------
# async def _init_dhan_client_into_engine_state(force: bool = False) -> None:
#     """
#     Initializes ENGINE_STATE['dhan'] using dhanhq SDK.
#     Auto-retries periodically if missing (Heroku startup race).
#     """
#     now = time.time()
...
#         ENGINE_STATE["dhan"] = None  # force recreate
...
#     except Exception as e:
#         ENGINE_STATE["dhan"] = None
#         logger.error(f"❌ Dhan SDK init failed (is dhanhq installed?): {e}")


# # -----------------------------
# # Marketfeed WS helpers
# # -----------------------------
# def _build_dhan_ws_url(access_token: str, client_id: str) -> str:
#     return (
#         f"{DHAN_WS_BASE}"
...
#     for k in ("data", "Data", "order", "Order", "result", "Result"):
#         sub = payload.get(k)
#         ap = _extract_avg_price(sub) if isinstance(sub, dict) else None
#         if ap is not None:
#             return ap
#     return None
...
# def _recompute_trade_from_exec(trade: Dict[str, Any], exec_price: float) -> None:
#     """
#     Your BreakoutEngine comment expects:
...
#         trade["entry_price"] = ep  # engine uses entry_price for pnl
...
# # -----------------------------
# # Dispatcher + Scanner + Candle builder + BreakoutEngine runner
# # -----------------------------
# def _push_scanner_signal(side: str, symbol: str, trigger_px: float, reason: str) -> None:
#     lst = STATE.scanner.get(side) or []
#     lst.insert(
...
#         # Ensure dhan client exists (FIX for "dhan session missing")
...
#         # per-tick volume delta from day volume (best effort)
#         v_delta = 0
#         if tick.volume is not None:
...
# # FastAPI
# # -----------------------------
# app = FastAPI(title="Nexus Core", version="2.2")
...
#     # keep engine config synced (breakout uses bull/bear)
#     if side in ("bull", "bear"):
#         ENGINE_STATE["config"][side] = payload or {}
...
#         # Prefer TradeControl.save_config if exists
#         save_fn = getattr(TradeControl, "save_config", None)
#         if callable(save_fn):
#             ok = await save_fn(client_id, access_token)
#             if not ok:
...
#         # Re-init dhan client immediately
...
# # -----------------------------
# async def _init_engine_live_flags() -> None:
#     # MUST be bools; BreakoutEngine uses bool(...) checks.
#     ENGINE_STATE["engine_live"]["bull"] = (STATE.engine_status.get("bull") == "1")
#     ENGINE_STATE["engine_live"]["bear"] = (STATE.engine_status.get("bear") == "1")
...
#         stocks[int(t)] = {
#             "token": int(t),
#             "security_id": int(t),  # your system assumes token == security_id
#             "symbol": sym,
...
#     # Init dhan client (may still fail if creds not yet saved)
#     await _init_dhan_client_into_engine_state(force=True)
...
#     logger.info("Shutdown complete")
# main.py  (FULLY INTEGRATED + FIXED FOR YOUR HEROKU dhanhq IMPORT)
#
# ✅ Fixes: "cannot import name 'DhanContext' from 'dhanhq'"
#    -> Uses: from dhanhq import dhanhq; dhan = dhanhq(client_id, access_token)
#
# ✅ Fixes: BreakoutEngine "dhan session missing"
#    -> Robust creds loader + auto re-init dhan client periodically
#
# ✅ Integrates:
#   - Dhan Marketfeed WS (ticks)
#   - 1-min candle builder from ticks (calls BreakoutEngine.on_candle_close)
#   - Tick-fast BreakoutEngine.run()
#   - Dhan Order Update WS worker + /api/stats flag
#   - Engine settings/toggles persisted to Redis
#
# IMPORTANT (Heroku):
#   Use ONE worker only, otherwise multiple websocket connections will cause disconnects:
#     Procfile:
#       web: uvicorn main:app --host 0.0.0.0 --port $PORT --workers 1
#
# requirements.txt must include:
#   fastapi uvicorn websockets pytz redis dhanhq
