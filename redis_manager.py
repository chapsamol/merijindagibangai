...
✅ Works with redis.asyncio (redis-py) correctly
✅ Heroku Compatible: Handles 'rediss://' TLS with self-signed certs
✅ Atomic Lua-based counters:
    - per-side daily trade limit
    - per-symbol daily max trades
    - open-position lock per symbol
...
def _redis_url() -> str:
    ...
    On Heroku, REDIS_TLS_URL is preferred for secure connections.
    """
    return (
        os.getenv("REDIS_TLS_URL")
        or os.getenv("REDIS_URL")
...
async def get_redis() -> redis.Redis:
    """
    Singleton async Redis client with safe concurrent init.
    Updated for Heroku SSL/TLS compatibility.
    """
    global _r
    if _r is not None:
        return _r
    ...
    async with _r_init_lock:
        ...
        # HEROKU TLS FIX:
        # Heroku Redis uses self-signed certificates for 'rediss://' protocol.
        # We must disable cert validation to allow the connection.
        if url.startswith("rediss://"):
            kwargs.update({
                "ssl_cert_reqs": None,
        ...
        try:
            ...
            await client.ping()
            _r = client
            logger.info("✅ Redis connected successfully (Heroku/TLS compatible).")
            return _r
        except Exception as e:
