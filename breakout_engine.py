...
class BreakoutEngine:
    ...
    def run(token, ltp, vol, state):
        ...
        if not stock or stock['symbol'] in state.get("banned", set()): return
        ...
        if stock['status'] == 'TRIGGER_WATCH':
            # Check Side Latch and verify limits via Redis LUA before opening
            side = stock['side_latch'].lower()
            if side == 'bull' and ltp > stock['trigger_px']:
                limit = state["config"]["bull"]["total_trades"]
                if TradeControl.can_trade("bull", limit):
                    BreakoutEngine.open_trade("bull", stock, ltp, stock['stop_base'], state)
            elif side == 'bear' and ltp < stock['trigger_px']:
                limit = state["config"]["bear"]["total_trades"]
                if TradeControl.can_trade("bear", limit):
    ...
    def analyze_candle_logic(token, candle, state):
        ...
        if state["engine_live"]["bull"] and candle['open'] < pdh < candle['close']:
            if BreakoutEngine.is_vol_qualified(stock, candle, 'bull', state):
                c_size = (candle['high'] - candle['low']) / candle['close']
                if c_size > 0.007:
                    if (candle['high'] - pdh) / pdh > 0.005: return
                    stock['stop_base'] = pdh * 0.9998
                else:
                    stock['stop_base'] = candle['low']
                stock['status'], stock['side_latch'], stock['trigger_px'] = 'TRIGGER_WATCH', 'BULL', candle['high'] * 1.0001

    @staticmethod
    def is_vol_qualified(stock, candle, side, state):
        matrix = state["config"][side].get('volume_criteria', [])
        if not matrix: return True
        c_vol, s_sma = candle['volume'], stock.get('sma', 0)
    ...
    def open_trade(side, stock, price, stop_base, state):
        ...
        risk_key = f"risk_trade_{min(stock.get('trades', 0) + 1, 3)}"
        ...
        state["trades"][side].append({
            ...
            "qty": qty, "side": "BUY" if side == 'bull' else "SELL", "step": risk_per_share * tsl, "pnl": 0.0
        })
        stock['status'] = 'OPEN'
        stock['trades'] = stock.get('trades', 0) + 1

    @staticmethod
    def monitor_active_trade(stock, ltp, state):
        for side in ["bull", "bear"]:
            state["trades"][side] = [t for t in state["trades"][side] if t['symbol'] != stock['symbol'] or BreakoutEngine.process_active(t, ltp, stock, state)]

    @staticmethod
    def process_active(trade, ltp, stock, state):
        trade['ltp'] = ltp
        trade['pnl'] = (ltp - trade['entry_price'] if trade['side'] == 'BUY' else trade['entry_price'] - ltp) * trade['qty']
        is_exit = False
        if trade['side'] == 'BUY':
    ...
    def apply_tsl(trade, ltp):
        ...
        if p >= trade['step']:
            new_sl = trade['entry_price'] + ((floor(p / trade['step']) - 1) * trade['step']) if trade['side'] == 'BUY' else trade['entry_price'] - ((floor(p / trade['step']) - 1) * trade['step'])
            if (trade['side'] == 'BUY' and new_sl > trade['sl_price']) or (trade['side'] == 'SELL' and new_sl < trade['sl_price']):
