...
# 1. IMPROVED SYMBOL MATCHING (Dhan uses various suffixes like -EQ, -BE)
# This replaces the logic to be much more aggressive in matching
old_match_logic = """                sym = (row.get(sym_col) or "").strip().upper()
                if sym in wanted:"""

# We search for the pattern even if it was partially patched before
pattern_to_replace = r'raw_sym = \(row\.get\(sym_col\) or ""\)\.strip\(\)\.upper\(\)\n.*sym = raw_sym\.replace\("-EQ", ""\)\n.*if sym in wanted:'
import re

new_match_logic = """                raw_sym = (row.get(sym_col) or "").strip().upper()
                # Advanced Dhan Match: strip all common suffixes
                sym = raw_sym.split('-')[0].strip()
                if sym in wanted:"""

if "sym = raw_sym.split('-')[0].strip()" not in content:
    if "raw_sym =" in content:
        # If already partially patched, we replace the block
        content = re.sub(r'                raw_sym = .*?if sym in wanted:', new_match_logic, content, flags=re.DOTALL)
    else:
        content = content.replace(old_match_logic, new_match_logic)
    print("✅ Symbol matching logic upgraded (Handles -EQ, -BE, -SM).")

# 2. FIX DEPRECATION WARNING (datetime.utcfromtimestamp)
old_dt = 'dt_utc = datetime.utcfromtimestamp(int(ts[i])).replace(tzinfo=pytz.UTC)'
new_dt = 'dt_utc = datetime.fromtimestamp(int(ts[i]), datetime.UTC)'

if old_dt in content:
    # Ensure datetime.UTC is available or use pytz.UTC
    content = content.replace(old_dt, new_dt)
    # Ensure 'datetime' is imported correctly for this syntax
    content = content.replace('from datetime import datetime, timedelta, date', 'from datetime import datetime, timedelta, date, timezone\nimport datetime as dt_mod')
    content = content.replace('datetime.UTC', 'timezone.utc')
    print("✅ Deprecated datetime calls fixed.")

# 3. FIX HEADER INCONSISTENCY & 401 ERRORS
if 'Content-Type": "application/json' not in content:
    content = content.replace('headers = {"access-token": access_token}', 'headers = {"access-token": access_token, "Content-Type": "application/json"}')

# 4. RESTORE FULL STOCK LIST (324 stocks)
FULL_STOCK_LIST = """STOCK_INDEX_MAPPING = {
    'MRF': 'NIFTY 500',
    '3MINDIA': 'NIFTY 500',
    'HONAUT': 'NIFTY 500',
...
if "360ONE" not in content:
    start_idx = content.find("STOCK_INDEX_MAPPING = {")
    end_idx = content.find("}", start_idx) + 1
    content = content[:start_idx] + FULL_STOCK_LIST + content[end_idx:]
    print("✅ 324 Stocks restored to mapping.")

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ sync_market_data.py fully patched for Dhan compatibility!")